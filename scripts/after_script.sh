...
runuser -l ubuntu -c 'cd /home/ubuntu/sample-nodejs && sudo pm2 start dist/src/server.js --name nodejs  --silent' >> /tmp/sample-nodejs-api-prod_deploy_logs
s1=`pm2 status | grep -we nodejs | awk '{print $12}'`
sleep 5
s2=`pm2 status | grep -we nodejs | awk '{print $12}'`
if [ $s1 == $s2 ]
then
